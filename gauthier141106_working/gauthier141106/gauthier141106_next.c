...
int main(void)
{
	...
	while(1)
	{
		//Toggle heartbeatsign for debugging
		rounds++;
		if (!rounds) PIND |= BUZZZ;

		//something has changed?
		status = (((PIND & PHOTZ) | (PINB & (PHOTY | PHOTX)) << 8) | (PINB & (DB25Z | DB25Y | DB25X)))
		if (status != oldstatus)
		{
